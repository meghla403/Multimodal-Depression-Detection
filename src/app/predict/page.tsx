      ...
      <div className="relative grid gap-10 md:grid-cols-[1.4fr_1fr] md:items-center">
        <div>
          ...
          <p className="mt-5 max-w-xl text-sm leading-relaxed text-muted sm:text-base">
            Drop in an audio sample, a short video clip, or both. The model
            extracts acoustic features with OpenSMILE, facial cues with
            MediaPipe, and fuses them via cross-modal attention — all in under
            two seconds.
          </p>
...
function TipsStrip() {
  const tips = [
    ...
    {
      icon: "mic",
      title: "Good audio matters",
      text: "Low background noise and natural speech improve acoustic feature quality.",
    },
    {
      icon: "clock",
    ...
    <section className="mt-10">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {tips.map((t) => (
          <div
            key={t.title}
            className="group flex items-start gap-3 rounded-xl border border-border bg-surface-2/50 p-4 transition-all duration-300 hover:-translate-y-0.5 hover:border-primary/40 hover:bg-surface-2"
          >
